...
### Fixed
- Installed package dependencies in the weekly `sync-external` workflow before invoking `node bin/cli.js list-external`, fixing clean GitHub runners that do not have `node_modules` available.

### Tests
- Added a regression test that verifies the `sync-external` workflow installs dependencies before running the CLI.

## [2.11.0] - 2026-06-24

### Added
- First-class Antigravity CLI (`agy`) platform support
- Global skill installation to `~/.gemini/antigravity-cli/skills/`
- Workflow-to-skill conversion for Antigravity CLI slash commands
- MCP installation to `~/.gemini/antigravity-cli/mcp_config.json`
- Cross-platform detection through the CLI config directory, `PATH`, and official macOS/Linux and Windows binary locations

### Changed
- Antigravity IDE detection now requires its IDE-specific config directory or application bundle instead of the shared `~/.gemini/` directory
- Workflow installation is capability-driven and reports the actual destination path
- Full uninstall removes agentools workflows that were converted to skills

### Documentation
- Added macOS, Linux, and Windows Antigravity CLI paths to the root README, package README, website, bundled agentools skill, and maintainer guide
